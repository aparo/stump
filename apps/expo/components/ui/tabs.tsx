...
const TabsList = React.forwardRef<TabsPrimitive.ListRef, TabsPrimitive.ListProps>(
	({ className, ...props }, ref) => (
		<TabsPrimitive.List
			ref={ref}
			className={cn(
				'web:inline-flex native:h-12 native:px-1.5 squircle h-10 rounded-lg p-1 tablet:h-14 items-center justify-center bg-background-surface',
				className,
			)}
			{...props}
		/>
		...
		return (
			...
			>
				<TabsPrimitive.Trigger
					ref={ref}
					className={cn(
						'web:whitespace-nowrap web:ring-offset-background web:transition-all web:focus-visible:outline-none web:focus-visible:ring-2 web:focus-visible:ring-ring web:focus-visible:ring-offset-2 squircle rounded-lg px-3 py-1.5 text-sm font-medium inline-flex items-center justify-center border border-transparent',
