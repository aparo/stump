...
mod tests {
	...
	async fn test_first_sync() {
		...
		let don_quixote_id = Uuid::new_v4();

		fake_data::Media {
			series_id: series.id.clone(),
			id: Some(don_quixote_id),
		...
		.insert(&db)
		.await;
		let robinson_crusoe_id = Uuid::new_v4();

		fake_data::Media {
			series_id: series.id.clone(),
			id: Some(robinson_crusoe_id),
		...
		let the_count_of_monte_cristo_id = Uuid::new_v4();

		fake_data::Media {
			series_id: series.id.clone(),
			id: Some(the_count_of_monte_cristo_id),
	...
	async fn test_pagination() {
		...
		let book_1_id = Uuid::new_v4();
		let book_2_id = Uuid::new_v4();
		let book_3_id = Uuid::new_v4();
		let book_4_id = Uuid::new_v4();
		let book_5_id = Uuid::new_v4();
		...
		assert_eq!(vec![book_1_id, book_2_id, book_3_id], first_page.media_ids);
		...
		assert_eq!(vec![book_4_id, book_5_id], second_page.media_ids);
	...
	async fn test_incremental_sync() {
		...
		assert_eq!(vec![book_1_id, book_2_id], sync_page.media_ids);
		...
		assert_eq!(vec![book_3_id, book_4_id], sync_page.media_ids);
		assert!(!sync_page.should_continue);
	}
	...
	async fn test_represent_new_entitlement() {
		...
		let sync_page = SyncPage::new(
			...
			&user,
			"sync_1234".to_string(),
			std::slice::from_ref(&new_book_id),
			0,
			10,
		...
		assert_eq!(new_book_id, ent.book_entitlement.id);
	}
	...
	async fn test_represent_changed_product_metadata() {
		...
		assert_eq!(book_id, ent.entitlement_id);
	}
	...
	async fn test_only_includes_epubs() {
		...
		let action_comics_id = Uuid::new_v4();
		fake_data::Media {
			series_id: series.id.clone(),
			id: Some(action_comics_id),
		...
		let voynich_manuscript_id = Uuid::new_v4();
		fake_data::Media {
			series_id: series.id.clone(),
			id: Some(voynich_manuscript_id),
		...
		assert_eq!(vec![don_quixote_id,], sync_page.media_ids,);
	}
}
