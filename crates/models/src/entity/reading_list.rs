...
fn get_reading_list_rbac_for_user(user_id: Uuid, minimum_role: i32) -> Condition {
	...
	let base_rbac = Condition::all()
		.add(reading_list_rule::Column::UserId.eq(user_id))
...
mod tests {
	...
	fn test_reading_list_rbac() {
		let condition = get_reading_list_rbac_for_user(
