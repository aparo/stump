...
impl MediaWithMetadataAndReadingSessions {
	pub fn find_by_id_for_user(id: Uuid, user: &AuthUser) -> Select<media::Entity> {
		let select = media::ModelWithMetadata::find_by_id_for_user(id, user);
		apply_reading_session_joins(select, user)
	}

	pub fn find_by_ids_for_user(ids: &[Uuid], user: &AuthUser) -> Select<media::Entity> {
...
impl ReadingState {
	...
	fn finished(media_id: Uuid, last_completed_at: DateTimeWithTimeZone) -> Self {
	...
	pub fn from_active_reading_session(media_id: Uuid, rs: &ReadingSession) -> Self {
	...
	pub fn from_media(m: MediaWithMetadataAndReadingSessions, book_url: String) -> Self {
		...
		let reading_state = match (
			...
			(_, Some(last_completed_at)) => {
				ReadingState::finished(media_id, last_completed_at)
			},
			(_, _) => ReadingState::unread(media_id),
		};
