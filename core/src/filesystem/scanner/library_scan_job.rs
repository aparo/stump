...
impl LibraryScanJob {
	pub fn new(id: Uuid, path: String, options: Option<ScanOptions>) -> Self {
		Self {
			id,
			path,
			config: None,
	...
	async fn init(
		...
		if library_is_missing {
			...
			ctx.emit_event(CoreEvent::DiscoveredMissingLibrary(
				event::DiscoveredMissingLibrary { id: self.id },
			));
	...
	async fn finalize(
		...
		match image_options {
			Some(options) if did_create || did_update => {
				...
				let params =
					ThumbnailGenerationJobParams::books_in_library(self.id, false);
				if let Err(e) = ctx
